...
export const BabyAnimation: React.FC<BabyAnimationProps> = ({ mode }) => {
  return (
    <div className="flex min-h-[190px] flex-col items-center justify-center px-[18px] pb-2 pt-4 transition-all duration-300">
      ...
      />
      <div
        className={`min-h-7 text-center font-pacifico text-base ${mode === 'happy' ? 'text-[#2a9d5c]' : mode === 'sad' ? 'text-[#e63946]' : 'text-ocean'}`}
      >
        {mode === 'happy'
          ? 'Yay! Baby is SO excited to meet you!'
          : mode === 'sad'
            ? 'Aww... baby will miss you!'
