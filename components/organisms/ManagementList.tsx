  ...
  return (
    <section className="overflow-hidden rounded-[28px] bg-white/90 p-[22px] shadow-card backdrop-blur-lg flex flex-col">
      ...
      <motion.div 
        ...
        initial="hidden"
        animate="visible"
        className="overflow-y-auto flex-1 max-h-[720px] pr-1 space-y-3"
      >
        <AnimatePresence mode="popLayout">
          {items.length > 0 ? (
              ...
              if (type === 'gifts') {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      {onToggleReservation && (
                        <span 
                          className={`text-[.72rem] px-2 py-0.5 rounded-full font-extrabold flex items-center gap-1 cursor-pointer transition ${gift.reserved ? 'bg-[#d4f7e5] text-[#1a6640]' : 'bg-seafoam text-ocean hover:bg-ocean hover:text-white cursor-pointer'}`}
                      ...
                      {onEdit && (
                        <button onClick={() => onEdit(index)} className="p-2 rounded-lg bg-seafoam/60 hover:bg-ocean hover:text-white transition text-ocean cursor-pointer" title="Edit Item">
                      ...
                      <button onClick={() => onDelete(index)} className="p-2 rounded-lg bg-[#ffe0e0] hover:bg-[#ffe0e0] hover:text-[#b91c1c] text-[#b91c1c] transition cursor-pointer" title="Delete Item">
                ...
                return (
                  <motion.div 
                    ...
                    animate="visible"
                    exit={{ opacity: 0, scale: 0.95 }}
                    className="flex items-center justify-between p-3 bg-white rounded-xl shadow-soft border border-ocean/10"
                  >
                    <div>
                    ...
                    <button onClick={() => onDelete(index)} className="p-1 rounded bg-[#ffe0e0] hover:bg-[#ffe0e0] hover:text-[#b91c1c] text-[#b91c1c] cursor-pointer">✕</button>
                  </motion.div>
                );
              }
            })
            ...
            <motion.div 
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="p-6 text-center text-sm font-bold text-[#497184] border-2 border-dashed border-[#b8e8f5] rounded-2xl bg-white/50"
            >
